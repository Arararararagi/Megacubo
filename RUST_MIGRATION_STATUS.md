...
- **EPG auto-refresh + scheduled refresh**: on launch, the app refreshes the XMLTV guide for every list with an `epg_url` wired (background task); a **periodic loop** then re-refreshes on the configured `epg_update_interval_secs` (default 1800s), both gated by the `auto_update_epg` setting. Progress is pushed to the UI via a `epg-progress` Tauri event (`start`/`list`/`list-done`/`error`/`done`) shown in the Channels toolbar. `remove_list` deletes a playlist and all its channels; UI has a **Remove** button.
...
- Features: **add playlist** (M3U URL + optional EPG URL, type selector M3U/Xtream), **channel grid** per list with **Play / ▶ App / Bookmark / EPG** actions (plus **⏪ Catchup** when the channel supports time-shift), **search**, **bookmarks** and **history** lists, an **EPG schedule** view (current + upcoming programmes), a **Settings tab** (theme dark/light applied live, external-player override, hardware-accel toggle, EPG auto-update + interval), and a dedicated **Plex tab** (sign in with Plex → libraries → movies/series → seasons → episodes → play in-app or external). The **▶ App** button (shown only when the binary was built with the `media` feature) plays in-app via libmpv and shows a player bar (play/pause/stop, seek, volume). External **Play** launches the OS default / VLC. Playback records history; bookmarking records bookmarks. Channels persist in SQLite so they survive restarts (reloaded on launch).
...
- **EPG**: channels without a `tvg-id` won't match EPG data (XMLTV keyed by channel id). Startup + periodic auto-refresh and progress reporting are implemented (gated by `auto_update_epg`); recurring refresh uses a fixed interval (no smart idle detection yet).
...
- **Release**: `default` Cargo features are now `["desktop","media"]`, so `cargo tauri build` produces the full app (all commands + in-app libmpv playback) without extra flags. `icons/icon.icns` added and listed in `tauri.conf.json`. `cargo tauri build` produces `target/release/bundle/macos/Megacubo.app` and `Megacubo_0.1.0_aarch64.dmg`. macOS build is unsigned (quarantine on first launch; right-click → Open to run). Libmpv is linked dynamically, so it must be present at runtime (`brew install mpv`).
- **Read-only FS fix**: `Config::config_path()` and mpv's `watch-later-directory`/`screenshot-directory` now resolve under `dirs::data_dir()/Megacubo` (writable), so changing settings / in-app playback no longer fail with "Read-only file system (os error 30)" when launched from the packaged `.app`. Config lives at `~/Library/Application Support/Megacubo/config.json` (macOS).
- **In-app playback (libmpv) fix**: the old `libmpv` crate (2.0.1) only supports the libmpv **1.x** client API and rejected the system libmpv **2.0** with `VersionMismatch`, so `Mpv::new()` failed and the "▶ App" button reported "media feature disabled". Switched to `libmpv2` (v6, targets client API major 2) — `MpvPlayer::new()` now succeeds. `media` feature now pulls `libmpv2`; `init_player` surfaces a clear error if mpv can't initialize. Requires system libmpv 2.x (`brew install mpv`).
...
- **UI**: functional but minimal (vanilla JS, no framework, no video embedding inside the webview — libmpv opens its own native window). Settings, catch-up picker, Plex tab and player bar are present. Intended as a usable baseline, not final design.
